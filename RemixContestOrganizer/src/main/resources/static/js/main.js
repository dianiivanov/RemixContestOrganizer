$(document).ready(function() {
    $("#locales").change(function () {
        var selectedOption = $('#locales').val();
        if (selectedOption != ''){
            window.location.replace(window.location.pathname + '?lang=' + selectedOption);
        }
    });

    // Custom file input init
    bsCustomFileInput.init();

    // Picture modal
    $('#exampleModal').on('show.bs.modal', function (event) {
        var button = $(event.relatedTarget); // Button that triggered the modal
        var url = button.data('image-url'); // Extract info from data-* attributes
        var title = button.data('image-title'); // Extract info from data-* attributes
        var modal = $(this);
        modal.find('.modal-title').text(title);
        modal.find('.modal-body img').attr('src', url);
        modal.find('.modal-body img').attr('alt', title);
    })
});
    let addRow = function (property) {
            let listName = property + 's'; //list name in Recipe.class
            let fieldsNames = ['id', property + 'Name']; //field names from Product.class
            let rowIndex = document.querySelectorAll('.'+ property +'-item').length; //we can add mock class to each movie-row
            let row = document.createElement('div');
            row.classList.add('col-sm-10', property + '-item');

            fieldsNames.forEach((fieldName) => {
                let input = document.createElement('input');
                if(fieldName === 'id'){
                    input.type='hidden';
                }else{
                    input.type = 'text';
                }
                input.classList.add("form-control");
                input.id = listName + rowIndex + '.' + fieldName;
                input.setAttribute('name', listName + '[' + rowIndex + '].' + fieldName);

                row.appendChild(input);
            });

            document.getElementById(property+'List').appendChild(row);
    };
